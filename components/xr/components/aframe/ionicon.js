...
AFRAME.registerSystem('ionicon', {
    init: function() {
      this.createdIcons = new Map();
    },

    addIcon: function(icon, fontColor=key_offwhite) {
      if (this.createdIcons.has(icon)) {
        return this.getIconID(icon);
      }
      // dimensions
      var multiplier = 350;
      var canvasWidth = multiplier;//this.data.height*multiplier;
      var canvasHeight = multiplier;//this.data.height*multiplier;
      ...
      drawIcon(ctx, canvas, icon, fontColor, 1);

      this.createdIcons.set(icon, canvas.id);
      return canvas.id;
    },

    getIconID: function(icon) {
      return this.createdIcons.get(icon);
    },
...
AFRAME.registerComponent('ionicon', {
    ...
    init: function () {
      ...
      var iconID = this.system.addIcon(this.data.icon);
      ...
      iconEntity.setAttribute('material', `shader: flat; src: #${iconID}; transparent: true; opacity: 1; side:front;`);
